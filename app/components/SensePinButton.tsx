...
export default function SensePinButton({ isPinned, onToggle, displayLocale, tutorialAttr }: Props) {
  ...
  return (
    <div className="group/pin relative shrink-0">
      ...
      >
        {isPinned
          ? <BsPinFill className="size-6 text-primary" />
          : <BsPin className="size-6 text-muted opacity-40 transition-opacity md:opacity-0 md:group-hover:opacity-100 md:group-hover/pin:opacity-100" />
        }
      </button>
