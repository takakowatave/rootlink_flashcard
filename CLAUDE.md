...
- **鉢植えの成長基準**: `score = quizCount + loginDays × 3` の単調増加スコアで5段階Lv。ロジックは `app/lib/plantGrowth.ts` に一元集約（PC/SP どちらも `PlantStatus` 経由で呼ぶこと。Dashboard等でハードコード禁止）。しきい値: Lv1=0 / Lv2=30 / Lv3=100 / Lv4=300 / Lv5=800（Lv5が最終形態）。アセットは `public/plant/lv1〜5.png`。UIの残pt表記は「あとNpt」で統一（「N問」は嘘、ログインでも増える）。仕様書は Notion「鉢植え成長ロジック」ページ https://app.notion.com/p/3acd9703217a81da9584dad6f8faf08a に同期
