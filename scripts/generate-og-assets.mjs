...
//   public/fonts/NotoSansJP-Regular-subset.ttf — 常用漢字+かな+ASCII サブセット
...
// Google Fonts の CSS API 経由で NotoSansJP-Regular の TTF URL を取得する。
// User-Agent を古めに指定すると unicode-range 分割されない単一 TTF を返してくる。
const GOOGLE_FONTS_CSS = 'https://fonts.googleapis.com/css2?family=Noto+Sans+JP:wght@400&display=swap'

async function fetchFontBinary() {
  const cssRes = await fetch(GOOGLE_FONTS_CSS, { headers: { 'User-Agent': 'Mozilla/5.0' } })
...
async function generateFont() {
  ...
  console.log(`Downloading NotoSansJP...`)
  const fontBuf = await fetchFontBinary()

  console.log(`Subsetting to ${chars.size} chars...`)
  const subset = await subsetFont(fontBuf, target, { targetFormat: 'truetype' })

  const outPath = path.join(ROOT, 'public/fonts/NotoSansJP-Regular-subset.ttf')
  await fs.writeFile(outPath, subset)
  const stat = await fs.stat(outPath)
  console.log(`✓ public/fonts/NotoSansJP-Regular-subset.ttf (${(stat.size / 1024).toFixed(1)} KB)`)
}
