...
- meaning_ja は短く端的に
- 日本語訳は自然な日本語（直訳調を避ける）
- **英語（meaning_en / explanation_en / example_en）はすべて British English で書く。**
  - 綴り: colour / centre / travelling / programme / behaviour など英式綴りを使う（color / center / traveling / program は不可）
  - `-ize` / `-yze` 系（organize, analyze, scandalize 等）は Oxford spelling として British English でも正なので使用可
  - 語彙: flat / lift / lorry / holiday / autumn 等の英式語彙を優先（apartment / elevator / truck / vacation / fall は使わない）
- JSON以外の文字（前置き・コードフェンス）は出力しない`

async function generatePayload(phrase, attempt = 0) {
  ...
  if (res.status === 429 && attempt < 3) {
    ...
    return generatePayload(phrase, attempt + 1)
  }
  if (!res.ok) {
...
async function markSkip(row, reason) {
  if (!row.id) return
  const { error } = await supabase.from('phrase_cards')
    .update({ skip_reason: reason, gate_checked_at: new Date().toISOString() })
    .eq('id', row.id)
  if (error) throw error
}
