...
### 8. posts INSERT を直接実行（下書きとして）

Supabase MCP `execute_sql` で直接 INSERT する。**下書き扱い** (`published_at = NULL`) 固定。kiko の確認 → 公開の順で必ず経由する。
...
markdown 本文は参考用に `/tmp/hp-{slug}-post.md` にも書き出す（プレビュールートで確認が本命だが、markdown ソース確認したい場合用）。

### 9. プレビュー案内

INSERT 後、kiko には `/blog/preview/{slug}` の URL を提示する。プレビューページに公開用 SQL が埋め込まれるので、kiko が確認 OK なら「公開して」と発話 → その後 Claude Code が `UPDATE posts SET published_at = NOW() WHERE slug = '...'` を execute_sql で実行する。

**kiko の確認前に自動で published_at を入れない**（=公開しない）。ここは必ず手動ゲート。
...
✅ posts INSERT (draft): slug=so-thats-why

- プレビュー: https://www.rootlink.app/blog/preview/so-thats-why
- 記事 markdown ソース: /tmp/hp-so-thats-why-post.md
- OK なら「公開して」と言ってください。UPDATE 走ります
```
...
- `posts` テーブルに直接 `published_at = NOW()` で INSERT する（必ず NULL の下書きで INSERT → プレビューで kiko 確認 → 明示の「公開して」指示を受けてから UPDATE で公開）
