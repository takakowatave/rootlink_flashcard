...
### 4. gate 合格 → Case B ペイロード生成
Notion spec の Case B に従う。BrE 前提。`locale` は原則 null（辞書ラベルが `British` / `American` と明記されているときのみ `en-GB` / `en-US`）。`register` は原則 `neutral`。

### 5. phrase_cards INSERT
Supabase MCP `execute_sql`（プロジェクトID `ybfdlsjuscgspkcgwist`）で INSERT。

```sql
INSERT INTO phrase_cards (
  phrase, meaning_ja, meaning_en, explanation_ja, explanation_en,
  example_en, example_ja, type, register, locale, gate_checked_at
) VALUES (...)
ON CONFLICT (phrase) DO NOTHING
RETURNING id;
```

既存 phrase の場合は `RETURNING` が空になるので `SELECT id FROM phrase_cards WHERE phrase = $1` で id を取り直す。

**重要**: `example_en` に HP固有名詞（Harry / Ron / Hermione / Hogwarts / Dumbledore 等）は絶対に入れない。中立的な日常シーンで作る。
